...
public struct TurboQuantBridge: Sendable {
    ...
    private func resolvePythonExecutable() throws -> URL {
        ...
        if let envPath = ProcessInfo.processInfo.environment["LLMCACHE_PYTHON"] {
    ...
    private func resolveHelperScript() throws -> URL {
        ...
        if let envPath = ProcessInfo.processInfo.environment["LLMCACHE_MLX_VLM_BRIDGE"] {
            return URL(fileURLWithPath: envPath)
        }
        ...
        guard FileManager.default.fileExists(atPath: helperURL.path) else {
            throw StoreError.notFound("mlx-vlm helper script not found at \(helperURL.path). Set LLMCACHE_MLX_VLM_BRIDGE to override.")
        }
        return helperURL
    }
}
