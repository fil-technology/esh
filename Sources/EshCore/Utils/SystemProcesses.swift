import Foundation
#if os(macOS)   // esh iOS M9: `SystemProcesses` shells out to `/bin/ps` (Process, macOS-only). Kept in EshCore
// as a surgical `#if os(macOS)` diff — it enriches the portable low-memory refusal message on macOS and is
// entirely absent from the iOS build, so no `Process` reaches the iOS path.
