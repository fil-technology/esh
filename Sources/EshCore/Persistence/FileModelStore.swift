...
public struct FileModelStore: ModelStore, Sendable {
    ...
    public func loadManifest(id: String) throws -> ModelManifest {
        ...
        let manifest = try manifestIO.read(from: url)
        return try normalized(manifest: manifest, manifestURL: url)
    }
    ...
    public func listInstalls() throws -> [ModelInstall] {
        ...
        return try FileManager.default.contentsOfDirectory(at: manifestsURL, includingPropertiesForKeys: nil)
            ...
            .map { try normalized(manifest: manifestIO.read(from: $0), manifestURL: $0).install }
    ...
    private func normalized(manifest: ModelManifest, manifestURL: URL) throws -> ModelManifest {
