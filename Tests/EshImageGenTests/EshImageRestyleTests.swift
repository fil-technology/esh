...
// Deterministic tests for the native image.RESTYLE provider (fast SDXL-Turbo img2img) wiring
// (discovery/progress/artifact/cancellation/errors) using a mock engine. This provider must advertise
// `image.restyle`, NOT `image.edit` — SDXL-Turbo regenerates from the prompt and does not preserve
// subject/scene, so it must never win the content-preserving instruct `image.edit` capability. The real
// MLX SDXL-Turbo path is validated on-device separately.
...
private func mockEdit(steps: Int = 3, png: Data) -> ImageRestyleFn {
...
private func editReq(imagePath: String?, prompt: String) -> ResolvedExecutionRequest {
    ...
    return ResolvedExecutionRequest(request: ExecutionRequest(capability: .imageRestyle,
...
@Suite struct EshImageRestyleTests {
    @Test func descriptorIsNativeImageRestyleNotInstructEdit() {
        let p = MLXImageRestyleProvider(modelID: "m", supported: true, edit: mockEdit(png: editPNG()))
        #expect(p.descriptor.capabilities == [.imageRestyle])
        // The guarantee: it must NOT advertise image.edit, so it never wins the instruct-edit capability
        // (which correctly falls through to the content-preserving Python instruct path where available).
        #expect(!p.descriptor.capabilities.contains(.imageEdit))
    ...
    @Test func editsImageWithProgress() async {
        let p = MLXImageRestyleProvider(modelID: "m", supported: true, edit: mockEdit(png: editPNG()))
        let out = await collectEdit(p.execute(editReq(imagePath: tmpImage(), prompt: "watercolor painting style"), context: editCtx()))
    ...
    @Test func missingInstructionFailsHonestly() async {
        ...
        #expect(out.artifacts == 0 && (out.failed?.contains("prompt") ?? false))
    }

    @Test func discoveryRequiresDownloadThenReady() async {
        let p = MLXImageRestyleProvider(modelID: "m", supported: true, edit: mockEdit(png: editPNG()))
        if case .requiresDownload = p.reportedAvailability(for: .imageRestyle) {} else { Issue.record("expected requiresDownload initially") }
        _ = await collectEdit(p.execute(editReq(imagePath: tmpImage(), prompt: "green"), context: editCtx()))
        if case .ready = p.reportedAvailability(for: .imageRestyle) {} else { Issue.record("expected ready after a run") }
    }

    @Test func unsupportedReportsHonestly() async {
        let p = MLXImageRestyleProvider(modelID: "m", supported: false, edit: mockEdit(png: editPNG()))
        if case .unsupportedOnPlatform = p.reportedAvailability(for: .imageRestyle) {} else { Issue.record("expected unsupportedOnPlatform") }
    }

    @Test func fileURIInputResolves() {
        let path = tmpImage()
        let inputs: [CapabilityInput] = [.attachment(EshAttachment(kind: .image, uri: "file://" + path)), .text("x")]
        #expect(MLXImageRestyleProvider.imageInputPath(inputs) == path)
    }

    @Test func paramsDefaultsSuitTurbo() {
        let d = MLXImageRestyleProvider.params(from: [:])
        #expect(d.strength == 0.7 && d.steps == 4 && d.maximumEdge == 768)
        let o = MLXImageRestyleProvider.params(from: ["strength": .double(0.4), "steps": .int(2), "maximumEdge": .int(1024)])
        #expect(o.strength == 0.4 && o.steps == 2 && o.maximumEdge == 1024)
    }

    @Test func providersFactoryYieldsNativeEditProvider() {
        let ps = EshImageRestyle.providers()
        #expect(ps.count == 1)
        #expect(ps.first?.descriptor.capabilities == [.imageRestyle])
    ...
    @Test func providersAcceptSelfHostedSDXLTurbo() {
        ...
        let ps = EshImageRestyle.providers(selfHosted: .sdxlTurbo())
        #expect(ps.first?.descriptor.capabilities == [.imageRestyle])
        #expect(ps.first?.descriptor.backend == .mlx)
    }
