...
| **project.generate** (text → multi-file site) | 🟢 done (live) | Text → validated self-contained **multi-file** static project (index.html + style.css + script.js, relative paths) as a `.webProject`, same isolated sandbox. `ProjectValidator` = safety (drops traversal/absolute/`..`/`~`, flags external resources) **+ quality** (requires real index.html, rejects `...`/placeholder content → retry/escalate, never saves garbage). Artifacts served with **per-file content types** (text/css, text/javascript) so strict browsers apply CSS/execute JS. Tier-0 routing (multi-file/web project vs single-file web/SVG/chat, false-exec still 0). **Fit finding:** Apple FM (~4 K ctx) fits small 2-3 file projects, overflows on larger output → pin a larger local code model (qwen3.5-9b) for big projects. Live: 3-file bookshop site in ~25s. **Framework/managed-runtime tier (Next.js/Three.js + npm/dev-server) DEFERRED** (untrusted Node execution = separate heavy/security-sensitive tier). |
