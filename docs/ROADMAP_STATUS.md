...
| **webArtifact.generate** (text → HTML) | 🟢 first slice done (live) | Text → validated self-contained HTML `.webProject`, isolated sandboxed-iframe preview; LLM codegen (Apple FM + repair), no heavy models; Tier-0 routing (web page vs SVG/image/chat, false-exec still 0); Web inline preview + Open/Download + inspector. Live: coffee-shop landing page in ~35s. Iterative editing with provenance lineage done. |
| **project.generate** (text → multi-file site) | 🟢 **PRODUCTION-READY** (live) | Text → validated self-contained **multi-file** static project (index.html + css + js, relative paths) as a `.webProject`, isolated sandbox. **Layer 1** safety+quality (drops traversal/absolute/`..`/`~`, flags external, rejects `...`/placeholder, requires real index.html). **Layer 2 `ProjectConsistency`** cross-file checks: JS→HTML targets (`#id` must exist), HTML/CSS→local assets exist+in-bundle, `.css`/`.js` pure (no `<style>`/`<script>` wrap) **and referenced** (no orphan/dead assets), structure (missing/unclosed `<body>`, dup ids), `{{templates}}`. **Bounded repair** (generate + ≤2 repairs, accept only if issues strictly reduce; else saved+marked invalid — never an unbounded agent loop). Structured stage provenance in `metadata.validation`; per-file MIME types (text/css, text/javascript). **Evidence-based model policy** (benchmarked): Apple FM only reliable local option (llama-3.2-3b cross-file-broken; deepseek-r1-7b ~8min+overflow; qwen3.5-9b crashes in mlx_lm — incompatible, NOT promoted). `ProjectComplexity.estimate` → Auto picks Apple FM for small, explains/clarifies (not raw 400) for large-with-no-fit; decision in `ExecutionPlan`+`metadata.selection`. Warm residency N/A (Apple FM = system service, cold≈warm ~24s). Tier-0 false-exec still 0. Live: landing/dashboard/catalog all valid, refs resolve, previews work. **Framework/managed-runtime tier (Next.js/Three.js + npm/dev-server) DEFERRED.** |
| **image.edit** (instruction editing) | 🟡 EXPERIMENTAL (architecture + safety proven; blocked on model) | `image + instruction → image` wired end-to-end: provider + routing (edit vs segment vs clarify, Tier-0 false-exec still 0) + typed artifact w/ license provenance + guarded `/v1/execute` (RAM guard killed a 768² run at 4.3 GB free — protects the machine) + Web before/after. **Not production**: open Qwen edit (~40 GB) too heavy for 32 GB, official Kontext is gated (needs HF license+token), feasible 9 GB community 4-bit Kontext produces garbage (format-incompatible). See `2_1_IMAGE_EDIT_MILESTONE_STATUS.md`. |
| **Stage 4.x** (deferred) | ⬜ not started | cross-model candidate enumeration for the scheduler; local video/music generation; remote runtime |
