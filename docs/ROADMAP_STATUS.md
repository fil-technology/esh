...
| **Capability Intent Router** (86eyucfbu) | 🟢 done (Router Auto pending) | Tier-0 deterministic + CapabilityIntent + independent validation + RoutingOutcome + Install-and-Resume (in-memory) + routing benchmark (seed: zero false executions); **wired** as `POST /v1/route` + `/v1/route/resume` and into web chat (route-first, install-and-resume card, plan inspector); **Tier-1** constrained semantic router (dynamic registry schema + resident-LLM, escalates on ambiguity, validated) live. Remaining: **Router Auto** benchmarking (Apple Foundation/FunctionGemma vs resident), LLM tool-call front door, multilingual/larger dataset, durable pending. See `2_1_CAPABILITY_ROUTER_STATUS.md`. |
