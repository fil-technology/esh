...
| **Capability Intent Router** (86eyucfbu) | ✅ done incl. Router Auto | Tier-0 deterministic + CapabilityIntent + independent validation + RoutingOutcome + Install-and-Resume (in-memory) + routing benchmark; `POST /v1/route`(+`/resume`, +`/benchmark`) + web chat wiring (route-first, install-and-resume card, plan inspector); **Tier-1** semantic router pluggable (resident-LLM / Apple FM / FunctionGemma, shared canonical schema). **Router Auto** (evidence-driven, explainable, conservative): measured all routers on the v2 multilingual (EN/RU/HE) dataset and keeps **Tier-0** — no Tier-1 is both safe (≤2% false-exec) AND beats the instant baseline (Apple FM 0.70 acc but 36% false-exec; FunctionGemma base capAcc 0; resident 3B capAcc 0). See `2_1_CAPABILITY_ROUTER_STATUS.md` + `2_1_ROUTER_FINETUNE_PROPOSAL.md`. Remaining: warm-router residency class, fine-tuned FunctionGemma, LLM tool-call front door, durable pending. |
