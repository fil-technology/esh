...
### Added
- **Runtime Lifecycle / Warm Pool (M7).** A backend-agnostic `RuntimeLifecycleManager` keeps model runtimes warm and reused across requests instead of loading + unloading each time. It tracks a loaded-model registry and per-model state (unloaded/loading/warm/active/idle/unloading/failed), deduplicates concurrent loads, enforces a unified-memory budget with a configurable safety reserve and a TTS reserve (text+speech coexistence), evicts idle/over-budget models (idle-timeout + memory-pressure, LRU) and refuses over-budget loads, bounds concurrency with interactive-over-background priority, and supports cancellation and prewarming. `esh serve` now runs with a shared warm pool. The Adaptive Scheduler is resource-aware: an already-warm model wins close calls (bounded bonus) and this is recorded in the rationale. Memory is honest about estimated vs measured. See RUNTIME_LIFECYCLE_REPORT.md. Follow-up: a persistent MLX bridge for weight-level residency.
