...
| — | Warm-TTS (per-call model reload) | Voice latency | Reload lives in `mlx-audio-swift`/TTSMLX; not esh-local | **Cross-repo, BLOCKED:** TTSMLX shipped v0.8.0 with the caching API, but **v0.8.0 does not compile** — `TTSModelRegistry` is referenced in `TTSModelStore.swift`/`TTSBackgroundModelDownloader.swift` but never defined (missing file in the tag). esh dep-bump reverted to 0.3.3 (builds clean). Awaiting a fixed TTSMLX release (0.8.1). Note: v0.7.0+ already caches — esh only needs one long-lived synthesizer once the package builds. Streamed sentence-pipeline (#13) is the shipped esh-local win. | _blocked on TTSMLX 0.8.1_ | — | — | n/a |
